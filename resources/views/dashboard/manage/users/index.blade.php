<x-dashboard.shell>
    ...
    <div x-data="{showModal:false}">
        <div class="overflow-auto rounded-lg border border-gray-200 shadow-md m-5">
            ...
            <form class="w-1/3 float-right m-4" action="{{route('manage.users')}}">
                        ...
                        <td class="px-6 py-2">
                            <div class="flex gap-2">
                                ...
                                @if($user->role()->first()->name != 'Admin')
                                    @if($user->status == true)
                                        <form action="{{ route('manage.users.suspend', $user->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="bg-red-50 p-1 px-2 rounded-md text-red-600 hover:text-red-900">Suspend</button>
                                        </form>
                                    @else
                                        <form action="{{ route('manage.users.activate', $user->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')

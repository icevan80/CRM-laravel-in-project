            ...
            <div class="w-full m-4 flex">
                ...
                <div class="w-1/2 mx-2">
                    ...
                    <div class="relative">
                        ...
                        <button type="submit"
                            class="text-white absolute right-2.5 bottom-2.5 bg-pink-600 hover:bg-pink-700 focus:ring-4 focus:outline-none focus:ring-pink-300 font-medium rounded-lg text-sm px-4 py-2">Искать</button>
                    </div>
                </div>
                ...
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col" class="pl-6 py-4 text-center font-medium text-gray-900 border p-2">
                            <input type="date" wire:model="selectedDay">
                        </th>
                        @for ($i = today()->setDateFrom($selectedDay); $i < today()->
                            setDateFrom($selectedDay)->addWeeks(2); $i->addDay(1))
                            <th scope="col" class="py-4 text-center font-medium text-gray-900 border p-2">{{
                                $i->isoFormat('MMM. D') }}<br />{{ $i->isoFormat('ddd') }}</th>
                            @endfor
                    </tr>
                    @for ($i = today()->setDateFrom($selectedDay)->hour(8); $i <= today()->
                        setDateFrom($selectedDay)->hour(20); $i->addMinutes(15))
                        <tr>
                            ...
                            @for ($k = $i->copy(); $k < $i->copy()->addWeeks(2); $k->addDay(1))
                                ...
                                @if($appointment->date == $k->toDateString() && ($appointment->timeSlot->start_time <=
                                    $i->toTimeString() && $appointment->timeSlot->end_time > $i->toTimeString()))
                                    @if($appointment->timeSlot->start_time == $i->toTimeString())
                                    <th rowspan="{{ today()->setTimeFromTimeString($appointment->timeSlot->end_time)->diffInHours($appointment->timeSlot->start_time) * 60  / 15 }}"
                                        wire:click="setSelectedAppointment({{ $appointment }})" scope="col"
                                        class="text-white bg-pink-600 text-center font-medium border p-2">{{
                                        $appointment->user->name }}</th>
                                    @endif
                                    @else
            ...
            <table
                ...
                <tbody class="divide-y divide-gray-100 border-t border-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 text-center">
                        ...
                        " colspan="9">Записей не нашлось</td>
                    </tr>
                    @else
                    @foreach ($appointments as $appointment)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 max-w-xs font-medium text-gray-700">{{ $appointment->timeSlot->start_time
